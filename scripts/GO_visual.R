...
library(ggplot2)
p <- ggplot(data = GO, aes(x = Term,y = Count, fill = Category)) + geom_bar(stat="identity", width=0.8) + coord_flip() +  xlab("GO term") + ylab("Gene counts") +scale_fill_manual(values = COLS)+ theme_test() + theme(axis.text = element_text(face = "bold", color = "gray50")) + labs(title = "GO_visualise")
ggpar(p)